...
public:
    ...
    iterator end() { return &vec[0 + rows*cols]; }
    const_iterator cbegin() { return &vec[0]; }
    const_iterator cend() { return &vec[0 + rows*cols]; }
...
Matrix<T> Matrix<T>::operator+(const Matrix& m) {
    if (this->rows != m.rows || this->cols != m.cols) {
        throw std::out_of_range("Incompatible matrix dimensions: index is out of range");
    }
...
void Matrix<T>::insert_row(unsigned int row) {
    if (this->rows == 0 || this->cols == 0 || row > this->rows) {
        throw std::out_of_range("Cannot carry out row operation: Index is out of range");
    }
    this->rows++;
    T* old_vec = this->vec;
    this->vec = new T[this->rows*this->cols];
...
void Matrix<T>::insert_col(unsigned int col) {
    if (this->rows == 0 || this->cols == 0 || col > this->cols) {
        throw std::out_of_range("Cannot carry out column operation: Index is out of range");
    }
